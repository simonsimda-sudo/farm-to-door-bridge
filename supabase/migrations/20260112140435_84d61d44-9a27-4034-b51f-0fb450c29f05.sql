-- 1) Fix overly strict name/city/country validation (allow full Unicode letters)
...
AS $function$
BEGIN
  -- Customer first name: 1-100 characters, letters/spaces/hyphens/apostrophes only (Unicode-safe)
  ...
  END IF;
  IF NEW.customer_first_name !~ '^[[:alpha:]][[:alpha:][:space:]''-]*$' THEN
  ...
  END IF;
  IF NEW.customer_last_name !~ '^[[:alpha:]][[:alpha:][:space:]''-]*$' THEN
  ...
  -- Delivery city: 1-100 characters, Unicode-safe letters/spaces/hyphens/apostrophes
  IF LENGTH(NEW.delivery_city) < 1 OR LENGTH(NEW.delivery_city) > 100 THEN
    RAISE EXCEPTION 'City must be 1-100 characters';
  END IF;
  IF NEW.delivery_city !~ '^[[:alpha:]][[:alpha:][:space:]''-]*$' THEN
...
-- 2) Remove accidental public-read access to customer PII (orders/order_items)
DROP POLICY IF EXISTS "Customers can view orders by token" ON public.orders;
DROP POLICY IF EXISTS "Anyone can view order items" ON public.order_items;

-- Keep public INSERT for guest checkout; keep admin SELECT/UPDATE policies as-is for now.